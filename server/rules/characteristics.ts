...
function affectsMatch(affects: AffectsFilter, src: GameObject, obj: GameObject, objSubtypes: string[]): boolean {
  ...
  // a lord's "other Goblins you control" also reaches a changeling and a permanent that IS its own chosen
  // type (Roaming Throne set to Goblin)
  if (affects.subtype) {
    const objDef = getDef(obj.defName)
    const isChangeling = (objDef.keywords ?? []).includes('changeling')
    const isOwnChosen = !!objDef.isChosenTypeItself && obj.chosenType === affects.subtype
    if (!objSubtypes.includes(affects.subtype) && !isChangeling && !isOwnChosen) return false
  }
  ...
  if (affects.subtypeChosen) {
    // changeling (CR 702.73) counts as the chosen type, and so does a permanent that IS its own chosen
    // type (Roaming Throne) — both are object-level facts, not printed subtypes
    const objDef = getDef(obj.defName)
    const isChangeling = (objDef.keywords ?? []).includes('changeling')
    const isOwnChosen = !!objDef.isChosenTypeItself && !!obj.chosenType && obj.chosenType === src.chosenType
    if (!(src.chosenType && (isChangeling || isOwnChosen || objSubtypes.includes(src.chosenType)))) return false
  }
  return true
}
