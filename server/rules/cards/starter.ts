...
export const STARTER_SET: CardDefinition[] = [
  ...
  // --- Coverage batch CARD39: top-200 singles that needed no new decision ---
  {
    ...
    spell: {
      targets: [{ kind: 'permanent', count: 1, filter: { controller: 'opponent' } }],
      effect: destroyTargetControllerFetchesLand({ basicOnly: true }),
    },
  },
  {
    ...
    freeIfCommander: true,
    spell: { effect: grantKeywordsToControlled(['indestructible'], { creaturesOnly: true }) },
  },
  {
    name: 'Windfall',
    types: ['Sorcery'],
    ...
    //  player discarded this way." (no choice to make — the whole hand goes, so no discard prompt)
    spell: { effect: windfall() },
  },
  {
    name: 'Ash Barrens',
    types: ['Land'],
    // "{T}: Add {C}." / "Basic landcycling {1} ({1}, Discard this card: Search your library for a
    //  basic land card, reveal it, put it into your hand, then shuffle.)" — landcycling has the same
    //  shape as channel (pay, discard this card from hand, get an effect), so it reuses that plumbing.
    abilities: [{ kind: 'activated', cost: { tap: true }, isMana: true, produces: ['C'], effect: addMana('C') }],
    channel: {
      cost: '{1}',
      label: 'Landcycle',
      effect: searchLibrary({ filter: 'basicLand', dest: 'hand', reveal: true, count: 1 }),
    },
  },
  {
    name: 'War Room',
    types: ['Land'],
    // "{T}: Add {C}." / "{3}, {T}, Pay life equal to the number of colors in your commanders' color
    //  identity: Draw a card." (see Cost.lifeFromCommanderColors for the identity simplification)
    abilities: [
      { kind: 'activated', cost: { tap: true }, isMana: true, produces: ['C'], effect: addMana('C') },
